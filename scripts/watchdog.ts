...
function withTaskLocale(payload: unknown, locale: 'zh' | 'en'): Record<string, unknown> {
  const taskPayload = toTaskPayload(payload) || {}
  const taskMeta = toTaskPayload(taskPayload.meta) || {}
  return {
    ...taskPayload,
    locale,
    meta: {
...
async function recoverQueuedTasks() {
  ...
  for (const task of rows) {
    ...
    try {
      const locale = resolveTaskLocaleFromBody(task.payload) || 'zh'
      const nextPayload = withTaskLocale(task.payload, locale)
      if (nextPayload !== task.payload) {
