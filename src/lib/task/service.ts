...
function readTaskLocale(payload: unknown): string | null {
  const payloadObject = toObject(payload)
  const payloadMeta = toObject(payloadObject.meta)
  return normalizeLocale(payloadMeta.locale) || normalizeLocale(payloadObject.locale)
}

function withTaskLocale(payload: unknown, locale: string): Record<string, unknown> {
  const payloadObject = toObject(payload)
  const payloadMeta = toObject(payloadObject.meta)
  return {
    ...payloadObject,
...
function preserveTaskLocale(payload: Record<string, unknown>, existingPayload: unknown): Record<string, unknown> {
  const locale = readTaskLocale(payload) || readTaskLocale(existingPayload)
  if (!locale) return payload
  return withTaskLocale(payload, locale)
}

async function repairTaskLocaleIfPossible(task: {
  id: string
  payload: unknown
}, fallbackPayload: unknown): Promise<Record<string, unknown> | null> {
  const locale = readTaskLocale(fallbackPayload)
  if (!locale) return null
  const nextPayload = withTaskLocale(task.payload, locale)
  await taskModel.update({
    where: { id: task.id },
    data: { payload: toNullableJson(nextPayload) },
  })
  return nextPayload
}
    ...
    if (existing) {
      if (isActiveStatus(existing.status)) {
        if (!hasTaskLocale(existing.payload)) {
          const repairedPayload = await repairTaskLocaleIfPossible(existing, input.payload)
          if (repairedPayload) {
      ...
      if (collided) {
        if (isActiveStatus(collided.status)) {
          if (!hasTaskLocale(collided.payload)) {
            const repairedPayload = await repairTaskLocaleIfPossible(collided, input.payload)
            if (repairedPayload) {
...
export async function tryUpdateTaskProgress(taskId: string, progress: number, payload?: Record<string, unknown> | null) {
  const nextPayload = payload
    ? preserveTaskLocale(
      payload,
      (await taskModel.findUnique({ where: { id: taskId }, select: { payload: true } }))?.payload,
    )
    : null
  const result = await taskModel.updateMany({
    where: activeTaskWhere(taskId),
    data: {
      progress,
      ...(nextPayload ? { payload: toNullableJson(nextPayload) } : {}),
    },
  })
  return result.count > 0
}
