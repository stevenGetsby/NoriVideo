...
export type TestStepName = 'models' | 'auth' | 'textGen' | 'imageGen' | 'credits' | 'audioGen'
...
async function testMediaKitProvider(apiKey: string): Promise<TestProviderResult> {
  await setProxy()
  const steps: TestStep[] = []

  try {
    const response = await fetch('https://mediakit.cn-beijing.volces.com/api/v1/tasks/amk-tool-enhance-video-probe', {
      headers: {
        Authorization: `Bearer ${apiKey}`,
      },
      signal: AbortSignal.timeout(15_000),
    ...
    if (response.status === 401 || response.status === 403) {
      steps.push({
        ...
        status: 'fail',
        message: `Authentication failed (${response.status})`,
        detail: responseText.slice(0, 500) || undefined,
      })
      return { success: false, steps }
    }
    ...
    if (response.status >= 500) {
      steps.push({
        name: 'auth',
        status: 'fail',
        message: `MediaKit provider error (${response.status})`,
        detail: responseText.slice(0, 500) || undefined,
      })
      return { success: false, steps }
    }
    ...
    steps.push({
      ...
      status: 'pass',
      message: response.ok
        ? 'MediaKit API Key valid'
        : `MediaKit API Key accepted (probe status ${response.status})`,
      detail: responseText.slice(0, 500) || undefined,
    })
    return { success: true, steps }
  } catch (error) {
...
export async function testProviderConnection(payload: TestProviderPayload): Promise<TestProviderResult> {
  ...
  switch (apiType) {
    ...
    case 'mediakit':
      return testMediaKitProvider(apiKey)
    case 'siliconflow':
