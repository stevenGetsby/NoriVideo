...
export function isCustomArtStyleValue(value: unknown): boolean {
 ...
 * @param fallbackPrompt - 自定义风格的回退 prompt（从项目的 artStylePrompt 字段获取）
...
export function getArtStylePrompt(
  ...
  if (!artStyle) return fallbackPrompt || ''
  const style = ART_STYLES.find(s => s.value === artStyle)
  if (style) return locale === 'en' ? style.promptEn : style.promptZh
  return fallbackPrompt || ''
}

export function resolveCustomArtStylePrompt(
  artStyle: string,
  locale: 'zh' | 'en',
  customStyles: CustomArtStyle[],
): string {
  const builtIn = ART_STYLES.find(s => s.value === artStyle)
  if (builtIn) return locale === 'en' ? builtIn.promptEn : builtIn.promptZh
  const id = artStyle.startsWith('custom:') ? artStyle.slice(7) : artStyle
  const custom = customStyles.find(s => s.id === id)
  if (custom) return locale === 'en' ? custom.promptEn : custom.promptZh
  return ''
}

export function parseCustomArtStyles(json: string | null | undefined): CustomArtStyle[] {
  if (!json) return []
  try {
    const parsed = JSON.parse(json)
    if (!Array.isArray(parsed)) return []
    return parsed.filter(
      (s: unknown): s is CustomArtStyle =>
        typeof s === 'object' && s !== null &&
        typeof (s as CustomArtStyle).id === 'string' &&
        typeof (s as CustomArtStyle).label === 'string' &&
        typeof (s as CustomArtStyle).promptZh === 'string' &&
        typeof (s as CustomArtStyle).promptEn === 'string'
    )
  } catch {
    return []
  }
