...
import { promptContextStorage } from '@/lib/prompt-i18n/prompt-context'
import { loadUserPromptOverrides } from '@/lib/prompt-i18n/user-overrides'
import {
...
export async function withTaskLifecycle(job: Job<TaskJobData>, handler: (job: Job<TaskJobData>) => Promise<Record<string, unknown> | void>) {
  ...
  try {
    ...
    const { result, textUsage } = await withTextUsageCollection(async () => {
      const overrides = await loadUserPromptOverrides(data.userId)
      return promptContextStorage.run(
        { userId: data.userId, overrides },
        () => handler(job),
      )
    })
