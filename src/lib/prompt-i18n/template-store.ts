...
import { getActivePromptOverrides } from './prompt-context'
...
export function getPromptTemplate(promptId: PromptId, locale: PromptLocale): string {
  ...
  // Check for user override from async context
  const overrides = getActivePromptOverrides()
  if (overrides) {
    const userTemplate = overrides[promptId]?.[locale] || overrides[promptId]?.['zh']
