...
export function buildPrompt(input: BuildPromptInput): string {
  const { promptId, locale, variables = {}, templateOverride } = input
  ...
  const template = templateOverride || getPromptTemplate(promptId, locale)

  const templatePlaceholders = extractPlaceholders(template)
  const defined = new Set(entry.variableKeys)

  // Only enforce strict catalog validation for default templates (not user overrides)
  if (!templateOverride) {
  ...
  for (const key of entry.variableKeys) {
    if (!(key in variables)) {
      // For user overrides, missing variables are OK (they may not use all placeholders)
      if (templateOverride) continue
      throw new PromptI18nError(
