...
import { PRIMARY_APPEARANCE_INDEX, isArtStyleValue, isCustomArtStyleValue } from '@/lib/constants'
...
export const POST = apiHandler(async (request: NextRequest) => {
    ...
    const resolvedAppearanceArtStyle =
        isArtStyleValue(normalizedArtStyle) || isCustomArtStyleValue(normalizedArtStyle)
            ? normalizedArtStyle
            : 'american-comic'
    ...
    const appearance = await prisma.globalCharacterAppearance.create({
        data: {
            ...
            changeReason: '初始形象',
            artStyle: resolvedAppearanceArtStyle,
    ...
    if (uploadDirect && allReferenceImages.length > 0) {
        // Direct upload: resolve each image to MediaObject, store media URLs
        const mediaRefs = await Promise.all(
            allReferenceImages.map((img) => resolveMediaRefFromLegacyValue(img))
        )
        const firstMedia = mediaRefs[0]
        const mediaUrls = mediaRefs.map((ref) => ref?.url || null).filter(Boolean) as string[]
        await prisma.globalCharacterAppearance.update({
            where: { id: appearance.id },
            data: {
                imageUrl: firstMedia?.url || null,
                imageMediaId: firstMedia?.id || null,
                imageUrls: encodeImageUrls(mediaUrls),
            }
        })
    } else if (generateFromReference && allReferenceImages.length > 0) {
