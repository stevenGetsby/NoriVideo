            ...
            <div className="relative aspect-[3/2] bg-[var(--glass-bg-muted)]">
                {displayImageUrl ? (
                    ...
                    <div className="flex h-full flex-col items-center justify-center px-4 py-6 text-[var(--glass-text-tertiary)]">
                        ...
                        />
                        <button
                            onClick={() => onImageEdit?.('character', character.id, character.name, 0, appearance.appearanceIndex)}
                            className="mt-2 flex items-center gap-1 text-xs text-[var(--glass-text-secondary)] hover:text-[var(--glass-tone-info-fg)] transition-colors"
                        >
                            <AppIcon name="edit" className="w-3.5 h-3.5" />
                            <span>{tAssets('video.panelCard.editPrompt')}</span>
                        </button>
                    </div>
                )}
                {isAppearanceTaskRunning && (
