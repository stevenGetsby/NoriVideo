...
export default function AssetsStage({
  ...
  const projectArtStyle = runtime?.artStyle ?? undefined
  const handleGenerateImage = useCallback(async (
    ...
    if (type === 'character' && appearanceId) {
      await generateCharacterImage.mutateAsync({ characterId: id, appearanceId, count, artStyle: projectArtStyle })
    } else if (type === 'location') {
      await generateLocationImage.mutateAsync({ locationId: id, count, artStyle: projectArtStyle })
    } else if (type === 'prop') {
      await propAssetActions.generate({ id, count, ...(projectArtStyle ? { artStyle: projectArtStyle } : {}) })
    }
  }, [generateCharacterImage, generateLocationImage, propAssetActions, projectArtStyle])
  ...
  return (
    <div className="space-y-4">
      ...
      <AssetsStageModals
        projectId={projectId}
        projectArtStyle={runtime?.artStyle ?? undefined}
        onRefresh={onRefresh}
