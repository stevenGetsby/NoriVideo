...
export default function NovelInputStage({
  ...
  const customStyleOptions = customStyles.map((s) => ({ value: `custom:${s.id}`, label: s.label }))
  const editingCustomStyle = editingCustomStyleId ? customStyles.find((s) => s.id === editingCustomStyleId) ?? null : null
  ...
  const handleDeleteCustomStyle = useCallback(async (value: string) => {
    ...
    if (artStyle === value) {
      onArtStyleChange?.('american-comic')
    }
  }, [artStyle, deleteStyle, onArtStyleChange])
  ...
  const handleSaveCustomStyle = useCallback(async (data: { label: string; promptZh: string; promptEn: string }) => {
    if (editingCustomStyleId) {
      ...
      onArtStyleChange?.(`custom:${newStyle.id}`)
    }
    setCustomStyleModalOpen(false)
    setEditingCustomStyleId(null)
  }, [editingCustomStyleId, updateStyle, addStyle, onArtStyleChange])
