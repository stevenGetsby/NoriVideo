...
export function useStoryboardModalRuntime({
  ...
  const lastEditPromptRef = useRef('')
  const lastEditImagesRef = useRef<string[]>([])

  const wrappedHandleEditSubmit = useCallback(
    async (prompt: string, images: string[], assets: SelectedAsset[]) => {
      lastEditPromptRef.current = prompt
      lastEditImagesRef.current = images
      return handleEditSubmit(prompt, images, assets)
    },
    [handleEditSubmit],
  )
  ...
  return {
    ...
    editingPanel,
    imageEditDefaults,
    lastEditPrompt: lastEditPromptRef.current,
    lastEditImages: lastEditImagesRef.current,
