...
export function usePanelCrudActions({
  ...
  const addPanel = useCallback(async (storyboardId: string, customData?: { description?: string; imageUrl?: string; videoPrompt?: string }) => {
    try {
      await createPanelMutation.mutateAsync({
        storyboardId,
        shotType: t('variant.defaultShotType'),
        cameraMove: t('variant.defaultCameraMove'),
        description: customData?.description || t('panel.newPanelDescription'),
        videoPrompt: customData?.videoPrompt || '',
        imageUrl: customData?.imageUrl || undefined,
        characters: '[]',
      })
      await onRefresh()
    } catch (error: unknown) {
