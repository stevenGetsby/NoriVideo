...
export default function StoryboardStage({
  ...
  // AddPanelModal state
  const [addPanelStoryboardId, setAddPanelStoryboardId] = useState<string | null>(null)

  const handleAddPanelClick = useCallback((storyboardId: string) => {
    setAddPanelStoryboardId(storyboardId)
  }, [])

  const handleAddPanelSubmit = useCallback(async (data: { description: string; imageUrl?: string; videoPrompt?: string }) => {
    if (!addPanelStoryboardId) return
    await addPanel(addPanelStoryboardId, data)
    setAddPanelStoryboardId(null)
  }, [addPanelStoryboardId, addPanel])
  ...
  return (
      ...
      >
        ...
        <StoryboardCanvas
          ...
          onAddPanel={handleAddPanelClick}
        ...
        {modalRuntime.editingPanel && (
          <ImageEditModal
            ...
            defaultPrompt={modalRuntime.lastEditPrompt}
            defaultImages={modalRuntime.lastEditImages}
        ...
        {addPanelStoryboardId && (
          <AddPanelModal
            onSubmit={(data) => { void handleAddPanelSubmit(data) }}
            onClose={() => setAddPanelStoryboardId(null)}
          />
        )}
      </StoryboardStageShell>
  )
