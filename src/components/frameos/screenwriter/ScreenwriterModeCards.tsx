  ...
  if (variant === 'empty') {
    return (
      <div className="mx-auto flex max-w-[820px] flex-col gap-4">
        {cards.map((card) => {
          const disabled = card.implemented !== true
          return (
            <span key={card.key} title={disabled ? UNIMPLEMENTED_HINT : undefined}>
              <button
                type="button"
                data-mode-key={card.key}
                disabled={disabled}
                title={disabled ? UNIMPLEMENTED_HINT : undefined}
                onClick={() => {
                  if (!disabled) onSelect(card.key)
                }}
                className="group relative flex w-full items-center gap-5 overflow-hidden rounded-[16px] border border-[var(--fos-border-soft)] bg-[var(--fos-bg-2)] px-6 py-5 text-left transition-colors hover:border-[var(--fos-border-mid)] hover:bg-[var(--fos-bg-3)] disabled:cursor-not-allowed disabled:opacity-55 disabled:hover:border-[var(--fos-border-soft)] disabled:hover:bg-[var(--fos-bg-2)]"
              >
        ...
        return (
          <span key={card.key} title={disabled ? UNIMPLEMENTED_HINT : undefined}>
            <button
              ...
              className="flex min-h-[82px] items-center gap-3 rounded-[10px] border bg-[var(--fos-bg-2)] px-4 py-3 text-left transition-colors hover:bg-[var(--fos-bg-3)] disabled:cursor-not-allowed disabled:opacity-55 disabled:hover:bg-[var(--fos-bg-2)]"
              style={{
