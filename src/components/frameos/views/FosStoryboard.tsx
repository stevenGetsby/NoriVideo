...
function StoryboardSuccess({ data, sb, activeEp, onSwitchEp }: {
  ...
  return (
    <div className="sd3-page">
      <div className="sd3-header">
        <button className="sd3-back"><AppIcon name="chevronLeft" className="h-4 w-4" /></button>
        <span className="sd3-title">分镜设计</span>
        <span className="sd3-header-sep" />
        <span className="fos-meta-tag">{data.projectName}</span>
        <span className="fos-meta-tag">剧情模式</span>
        <span className="fos-meta-tag">精品版 2.0</span>
        <span className="fos-meta-tag">9:16</span>
        <span className="fos-meta-tag">仿真人</span>
      </div>
...
function StoryboardFailed({ data, episodes, activeEp, onSwitchEp }: {
  data: FosProjectData
...
export function FosStoryboard({ data }: { data: FosProjectData }) {
  ...
  return <StoryboardFailed data={data} episodes={episodes} activeEp={activeEp} onSwitchEp={setActiveEp} />
}
