...
export function useCharacterCreationSubmit({
  ...
  // Upload and return storage keys (for direct upload mode - persistent URLs)
  const uploadReferenceImagesAsKeys = useCallback(async () => {
    const uploadMutation = mode === 'asset-hub' ? uploadAssetHubTemp : uploadProjectTemp
    return Promise.all(
      referenceImagesBase64.map(async (base64) => {
        const data = await uploadMutation.mutateAsync({ imageBase64: base64 })
        if (!data.key && !data.url) throw new Error(t('errors.uploadFailed'))
        return (data.key || data.url) as string
      }),
    )
  }, [mode, referenceImagesBase64, t, uploadAssetHubTemp, uploadProjectTemp])
  ...
  // 直接上传模式：将用户上传的图片直接作为角色形象，跳过 AI 生成
  const handleUploadDirect = useCallback(async () => {
    if (!name.trim() || referenceImagesBase64.length === 0) return

    try {
      setIsSubmitting(true)
      const referenceImageUrls = await uploadReferenceImagesAsKeys()
