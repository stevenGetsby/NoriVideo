              ...
              <div className="ml-auto flex gap-0.5">
                {candidateData.candidates.slice(0, 4).map((_, index) => (
                  <button
                    key={index}
                    type="button"
                    onClick={(e) => { e.stopPropagation(); onSelectCandidateIndex(panel.id, index) }}
                    className={`relative h-6 w-6 flex items-center justify-center rounded-full transition-transform hover:scale-125`}
                    aria-label={`candidate-${index + 1}`}
                  >
                    <span className={`block h-3 w-3 rounded-full ${index === candidateData.selectedIndex ? 'bg-[var(--glass-accent-from)]' : 'bg-[var(--glass-bg-surface)]/80 border border-[var(--glass-stroke-base)]'}`} />
                  </button>
                ))}
              </div>
            </>
