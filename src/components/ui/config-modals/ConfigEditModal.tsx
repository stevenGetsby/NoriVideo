...
export function SettingsModal({
    ...
    const customStyleManager = useCustomArtStyleManager({
        ...
        onSelect: (value) => handleChange(onArtStyleChange)(value),
    })
    ...
    return (
        ...
        >
            <div className="glass-surface-modal p-7 w-full max-w-3xl transform transition-all scale-100 max-h-[90vh] flex flex-col">
                ...
                <div className="space-y-5 flex-1 min-h-0 overflow-y-auto app-scrollbar">
                    ...
                    {/* 提示词模板 */}
                    <div className="glass-surface-soft p-5 sm:p-6 space-y-4">
                        <h3 className="text-sm font-semibold text-[var(--glass-text-tertiary)]">提示词模板</h3>
                        <p className="text-xs text-[var(--glass-text-tertiary)]">自定义剧本拆分、分镜生成、台词分析等步骤的 AI 系统提示词</p>
                        <button
        ...
        {showPromptEditor && (
            <PromptTemplateEditor onClose={() => setShowPromptEditor(false)} />
        )}
